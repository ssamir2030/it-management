...
import { UserCog, CheckCircle, Clock, History } from "lucide-react"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"

interface CustodyStatsProps {
    totalItems: number
    activeItems: number
    returnedItems: number
}

export function CustodyStats({ totalItems, activeItems, returnedItems }: CustodyStatsProps) {
    const stats = [
        {
            title: "إجمالي العهد",
            value: totalItems,
        ...
        {
            title: "نسبة الإرجاع",
            value: totalItems > 0 ? `${Math.round((returnedItems / totalItems) * 100)}%` : "0%",
            icon: CheckCircle,
