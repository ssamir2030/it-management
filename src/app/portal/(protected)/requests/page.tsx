...
export default async function RequestsPage() {
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            ...
            case 'NEEDS_PURCHASE': return <Badge variant="outline" className="bg-orange-50 text-orange-700 border-orange-200">بانتظار الشراء</Badge>
        ...
        <div className="space-y-8">
            <PremiumPageHeader
                ...
                stats={[
                    ...
                    { label: "بانتظار الشراء", value: requests.filter(r => r.status === 'NEEDS_PURCHASE').length, icon: Box },
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-muted-foreground mt-1 line-clamp-1 dark:text-slate-400">
                                                {req.details ? req.details.replace(/:::DATA:::[\s\S]*?:::END_DATA:::/g, '').replace(/<!-- DATA: .*? -->/g, '').replace(/<!-- USER_NOTES_START -->/g, '').trim() : 'لا توجد تفاصيل'}
                                            </p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-4">
